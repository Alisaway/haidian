...
- 五组 `assets/figures/*.png` 的原始图件从提交 GeoJSON、指标和结构化矩阵确定性绘制；v1.5重点片区图同时读取 `visual/assets/spatial-prototypes.json` 和 `preimplementation-package.json`。12米×8米P0是参赛者参考测试装配，不是实测平面、场地定位或工程无障碍认证。
...
- v1.5四份10页PDF均由本地确定性ReportLab生成器重建，使用同一OFL字体的400/700静态实例嵌入。重点区与预实施图件由原创矢量概念图光栅化，不引入第三方图像。
- 本包采用 `COMMUNITY-DISPLAY-ONLY` 声明；仓库与组织方的最终许可条款优先。
- `assets/media/cjk-html-proposal.jpg`与`cjk-html-visual.jpg`保留为72275d8e8版本历史截图，不作为v1.5中文HTML视觉验收证据。当前版本仍需重新完成人工截图验收。
