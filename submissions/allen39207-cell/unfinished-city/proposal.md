...
当前可复算的指标来自提交几何：临时设计范围面积约 11.41 平方公里、三处重点区、10 个场景节点与四类城市设施。绿地、公共空间、建筑基底等数值用于概念层内部一致性校核，正式边界发布后必须重算；法定强度指标保持待确认。[metric:site_area_sqm] [metric:green_ratio] [metric:public_space_ratio]
