...
| **伦敦 King's Cross** | 67 英亩铁路场站废弃地 | 建筑师自述"把伦敦丢失的一块缝回城市肌理"；**40% 用地给公园街道与公共空间**，修复 20 栋历史建筑，新增 20 条街道 | **先文化锚点后科技巨头**——中央圣马丁先进驻，谷歌 Meta 后到；40% 可负担住房是制度性防线 | 可负担住房 40% 在"承诺"与"已建成"口径间有出入，本方案并列呈现不择一 |
| **巴塞罗那 22@** | 波布雷诺旧工业区，2000 年启动 | 用地重新分类，工业转知识密集产业 | 用地分类改革是启动器 | **最重的一课**：居民协会前负责人称其把街区大部分小工业"抹掉了"，十年房价每㎡涨近 3000 欧元 |
...
**二、算力应接入而非自建。** 北京人工智能公共算力调度平台已在建设跨域调度体系。物流半径长意味着本带需要的是**算力接入点与调度界面**，不是每个片区自建机房。**这直接影响用地——避免为不必要的机房预留过多用地。**
...
>
> **口径修正说明**：五道口空置率早期按"五道口大区"平均口径为 13%。清华科技园约 69 万㎡、入驻率近 98%（空置约 2%），是片区最大单体且几乎满租；空置主要集中在成府路沿线乙级楼宇、五道口商务区及新交付／改造中项目，而非科技园本身。现按园区核心口径修正为约 6%。
...
> 以下 4.6–4.11 小节仍是待填正文的要点骨架（用户画像/场景卡体系尚未收到 `personas-scenarios.md` 对应的完整正文，城市风貌/指标体系/运营机制同理），编号已从原 3.x 顺接到本节 4.x 序列，避免与上面 4.1–4.5 的真实正文重复编号；原 3.2 城市更新总体框架骨架与新写入的 4.2 内容重复，已删除。

### 4.6 用户画像与场景卡体系
- 两条自律前置：场景须落到具体位置（片区/梯度带/人字单元）；场景须给出"人在环中"的否决权或裁量权落点
- 六类画像 P1–P6：在读研究者／早期创业者／机器人测试工程师／世居居民／企业服务与资本从业者／研学与遗产访客——每类含代表人群、活动范围、核心诉求、最怕的事
...
### 4.7 城市风貌
- 城市基调："自主"的百年接力——京张铁路自主设计到 AI 全栈自主创新，不复古不未来主义，"折返"动作贯穿
- 管控引导要求表：高度（主导 5–7 层，遗址线两侧 200 m 内不超 6 层）／强度（不设容积率上限，官方指标未公开）／风貌材料（砖、混凝土、钢）／屋顶折面／体量（沿遗址线鼓励断开，保证横向视线通廊）
- 清河与小月河景观风貌系统："两横一纵"蓝绿骨架（遗址线纵、两条水系横）
- 北影等艺术资源利用（此前缺口，此处补上）：影像作为 AI 场景／公共空间影像界面／历史叙事影像化
- 标志性城市景观节点调和：南端展线台／北端清河台／上跨五环节点／中段零公里／跨学院路人字桥
...
- B 类承诺指标五组（写入本节正文，作为公开承诺，非机器复算）：
  - 反置换组——小微业态五年留存率 ≥70%／人才可负担住房比例 ≥40%／租金年涨幅上限 ≤5%／公共空间占比 ≥40%
...
- **算力**：设算力接入点与调度界面，**不自建独立算力池**（依据见第 3.2 节四流半径分析）
...
五道口·清华科技园空置率 6%，清华科技园约 69 万㎡、入驻率近 98%。**市场已经充分认可这个地方，强行大规模更新是破坏而非改善。**
...
> 22@ 的教训在此最直接：**当地居民协会前负责人表示该计划把街区里大部分小工业抹掉了。** 本片区若重蹈此辙，方案按其自身标准即为失败。
...
本节是本方案的立场落地处。任务书要求不少于 10 张 AI 场景卡、不少于 3 个产业测试验证场景、不少于 5 类用户画像，以及场景—空间—运营映射，并重点回应小月河场景赋能翼与公共体验路径 [source:AGENT-TASKBOOK] [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK]。本方案提供 **6 类画像、17 张场景卡、3 类测试验证场景**，并对全部场景执行两条自律。
...
算力设施是本项目必然出现的基础设施，余热不利用即为浪费与热岛。AI 负责负荷预测与热量调度。**人在环中：供热优先级由社区与运营方共同约定，冬季民生用热优先于算力扩容**，调度参数公开可查。空间要求：算力设施与生活带之间的管廊路由；末端换热站结合社区服务设施设置。指标：年度回收热量；覆盖户数；民生优先条款执行记录。
...
22@ 的直接教训：当地居民协会前负责人表示该计划把街区里大部分小工业抹掉了，十年间房价每平方米上涨近 3000 欧元。**AI 的角色：无。人在环中：留改决策由包含既有经营者的委员会作出，不由开发主体单方决定。** 空间要求：存量改造中划定**小微业态保留区**，租金调整设上限；改造期间提供过渡经营空间。指标：**既有小微业态五年留存率（本方案核心反置换指标）**；租金年涨幅上限执行情况。
...
**每一级都必须有明确的准入门槛与退出评估**，否则驻留会变成低价工位而非孵化机制。算力配额应接入北京人工智能公共算力调度平台，而非在本带内另建独立算力池。
...
**二、算力接入而非自建。** 本带需要的是算力接入点与调度界面，不是每个片区自建机房。**这直接减少了对大型市政设施用地的需求。**
...
> 说明：本节与第四节 §4.7 城市风貌主题重叠（同源于 `renewal-character.md`），此处按官方要求提供完整的蓝绿骨架 + 城市基调 + 管控引导 + 公共空间组件库正文；§4.7 保留原有骨架，两处内容基本一致，后续如需去重可将 §4.7 改为指向本节的简要引用——本轮先如实并存，不擅自删减任一处。
...
| 要素 | 引导要求 | 依据 |
|---|---|---|
| **高度** | 主要建设区以 **5–7 层**为主导层数；遗址线两侧 200 m 内不宜超过 6 层 | 对 50 个创新区的研究显示 5–7 层是创新活动的最佳密度——足够密以聚人，又低到能产生人尺度互动 |
| **强度** | **不设统一容积率上限**（官方管控指标未公开），以层数与贴线率共同控制体量 | 避免在数据缺失下编造指标 |
| **风貌** | 以砖、混凝土、钢三种材料为基调，呼应铁路工业遗产的材料谱系；**禁止仿古与过度科技感包装** | 京张铁路的材料本体 |
| **屋顶形态** | 遗址线两侧宜采用**折面**（单折或双折），呼应人字动作；第五立面纳入管控 | 图形与空间同源 |
| **体量** | **单体面宽不宜过长，沿遗址线一侧鼓励断开，形成横向视线通廊** | 保证横向缝合的视线可能 |
...
| **公共空间** | 公共空间占总用地比例设下限，作为更新准入条件 | 公共空间占比 |
...
| 保留区商户租金年涨幅上限 | **≤ 5%** | 需与国内政策环境校核，目前为原则性设定 |
| 公共空间占总用地比例 | **≥ 40%** | 引自 King's Cross 的 40% 用地用于公园、街道与公共空间 |
...
| 分级 | 指标 | 来源 |
|---|---|---|
| **有案例出处** | 40% 可负担住房、40% 公共空间 | King's Cross |
| **由失败案例反推** | 70% 留存率、0.6 季节均衡度 | 22@、首尔路 7017 |
| **本方案自设，无外部依据** | ≥8 处缝合单元、60% 工位覆盖、800 m 三角边、100 天活动、5 类机构、5% 租金上限、20 条年度收录 | 由方案逻辑推导 |
...
**本方案全部文字、图纸、图表、几何数据与代码均为本次征集的原创产出。**

- **未使用**未经授权的字体、图片、商标、人物肖像或企业标识
- **未使用**受版权保护的论文图像、摄影作品或地图底图
- 所有图纸与图表由本方案的参数化脚本生成，中英双语由同一脚本一次输出。受征集提交规则限制（提案目录仅接受指定文件类型），脚本不随包提交，可按主办方要求另行提供以供复核
- 引用的第三方数据（房地产服务机构报告、租房行情平台）仅使用其**数值与结论**，不复制其图表、文字表述或版式；来源与性质已在第 1 节声明
- 引用的公开政策文件与新闻报道，均以转述方式使用，**未整段复制原文**
- 生态图谱中的机构与企业名称为公开信息中的客观事实指称，不使用其商标图形或视觉标识

**关于 AI 参与的声明**：本方案由人工智能参与设计与撰写，模型信息已在 `agent.json` 中如实声明。**本方案主张这一事实应当被记录而非隐藏**——零公里碑刻体系的智能体贡献者收录（见第 2.5 节、S12）即是这一立场的空间表达。
...
1. 百年京张 AI 创新带城市设计项目官方公告 [source:OFFICIAL-ANNOUNCEMENT]
2. 面向智能体的开源征集任务书 [source:AGENT-TASKBOOK]
3. 城市设计管理办法（住房和城乡建设部）
4. 控制性详细规划编制相关规范（住房和城乡建设部）
...
8. 本次征集场地包及三层范围资料 [source:SITE-PACKAGE]
9. 处理后事实包与缺资料清单 [source:PROCESSED-FACT-PACK]
10. 海淀分片区租金与空置率估算——基于第一太平戴维斯、仲量联行、戴德梁行、高力国际报告及安居客、搜狐租房行情整理。**性质为区域估算，用于对比示意，非逐楼精确值**

### 国际对标案例

11. Station F（巴黎）——官方资料及相关建筑评论
12. King's Cross（伦敦）——开发方与建筑师公开资料
13. 22@Poblenou（巴塞罗那）——社会影响评论与居民组织公开表述
14. High Line / Hudson Yards（纽约）——联合创始人公开访谈及建筑评论
15. Hazelwood Green / Robotics Row（匹兹堡）——卡内基梅隆大学发布、项目官方网站、区域产业报道
16. Kendall Square（波士顿）——创新区形态学研究（建筑高度与创新活动关系）
17. 首尔路 7017（首尔）——学术批判性评估、项目资料及维护状况报道

### 本地既有机制

18. 中关村论坛及人工智能主题日公开信息
19. 北京人工智能创新街区相关公开发布
20. 北京人工智能公共算力调度平台建设公开信息
21. 京张铁路遗址公园一期建设公开报道

### 本方案自身产出

22. 全球对标研究（线性遗产 × 创新区，七案例）
23. AI 创新生态四流图谱（基于公开信息整理）
24. 图纸与图表生成脚本——所有图纸由参数化脚本生成，中英双语由同一脚本一次输出；受征集提交规则限制（提案目录仅接受指定文件类型），脚本不随包提交，可按主办方要求另行提供以供复核

> **引用方式声明**：以上第三方来源均以转述方式使用，未整段复制原文，未复制其图表、版式或视觉标识。数据类来源仅使用其数值与结论。详见第 12.4 节版权与素材声明。
