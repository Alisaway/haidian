...
本轮只补参与者当前能够诚实完成的可实施性证据：P0 参考平面与容量算术、三席班次、90 天与年度成本敏感性、维护/撤场周期、四个触发式备选，以及 30 秒/3 分钟/15 分钟评审入口。真实场地、主体、报价、保险、预算、专业签认和现场观察没有被代填，仍由原八门保持阻断。
...
`metrics.json` 共 **100 项指标，86 项已赋值、14 项待测**。86 项已赋值项来自 GeoJSON、合成演练、设计合同、机器校验或明确标注的参赛者预可行性输入；本轮新增 14 项只记录 P0 的控制/工作面面积、保守容量、三席班次、排队停止线、排期余量、成本/运维敏感性、四个备选和未实测撤场目标，不把任何一项写成正式场地、法定容量、真实报价、批准预算或实施绩效。故“8/8 需求夹具”“24 场计划下限”“约 12万—29万元敏感性”和“真实观察 0”分别记录设计覆盖、未来样本计划、参赛者试算与证据边界，绝不合并为用户测试或预算通过。14 项待测继续包括法定强度、真实交接时间、返工轮次、公众接受度、现场无障碍和运营效果，不以推测填空 [depth:metrics_recalculation]。
