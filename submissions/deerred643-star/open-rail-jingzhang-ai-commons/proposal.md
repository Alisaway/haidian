...
> | 表达完整性 | **中英双语全量成果、离线可复现，图面带真实肌理、三级层级与全线—分段多联构图**：正文/HTML/A3/A0/图件/视频＋字幕＋文稿；程序绘制原创图件，全部平面图件叠入概念建筑肌理基底（1107 个基底）并以「背景肌理—结构—焦点」三级层级组织；总体图为「全线全览＋三段放大」多联构图（K0–9.7 里程刻度＋街区级细读），另附主线体验长卷呈现步行序列 | ✓ 双语成果逐项对应；离线渲染可复现并有覆盖证据；本表即为评审入口索引 |
