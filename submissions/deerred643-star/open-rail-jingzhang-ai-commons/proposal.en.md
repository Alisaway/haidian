...
> | Feasibility | **Implementation economics as a first-class citizen**: five-element implementation cards for 8 renewal projects + trigger-gated sequencing methodology + six-type funding matrix + four-link accountability chain + lifecycle O&M ledger framework; the six data gaps (ownership, named operators, statutory controls, municipal capacity, costs, measured benefits) each carry a trigger-executable deterministic resolution path | ◐ Mechanism files ship in the package (`implementation-cards.json`, `sequencing-methodology.json`, `funding-matrix.json`, `lifecycle-om-framework.json` + new `operator-qualification.json`, `statutory-reconciliation.json`, `capacity-preassessment.json`, `parametric-cost-model.json`); benefit figures stay "pending baseline", no ranking conclusions output |
