...
这些图解由统一注册表、B2活动台账和B3场景/用户数据生成，全部标注为研究图解、暂定空间和未部署；中英对应图件保留同一ID、状态和信息架构。为避免生成图缺字、图意悬空或视觉精度替代事实精度，16组中英图件逐一登记“核心判断—京张场地依据—状态限制—证据入口”，见 `visual/assets/figure_explanation_register.json`。[data:visual/assets/canonical_registry.json] [data:visual/assets/data_asset_register.json]
