...
当前包仍保留`NO_SUBMIT`保护：经明确授权已准备个人fork、本地稀疏工作区与候选分支，但未创建commit，未push，未创建PR、Issue、评论或review，也未发生任何上游写入。当前官方本地校验状态只以`self_check.json`为准；只有四门均通过且预检完成后，才可进入人工PR授权节点。该状态是工作流边界，不是正式提交、评审或入选声明。
