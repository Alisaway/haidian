...
summary: "一条以午间自助使用为优先的京张公共路线：设计目标是让使用者无需预约、扫码、登录、专用设备或日常工作人员，仍能依靠实体空间与固定信息完成行走、停留、判断和离开；AI 只作可选解释，核验、维护与停止机制属于后台责任层。"
...
本轮只重排既有方案的阅读顺序，不新增任何事实主张。**30 秒**先看一位午间使用者如何“进入—步行—短停—判断—离开”，全程不必打开手机、登录系统或等待工作人员；**3 分钟**再看三区如何分别提供研究旁的公共通行、无设备的信息停留与到达后的自主判断；**15 分钟**最后核对 WP-01—06 与 Phase 0—3 如何在后台处理证据缺失、过期或不可审计时的 HOLD、改线、降级或撤除。所有地点关系、构件和条件仍是概念表达或待核验事项，不构成现状、许可或实施承诺。
