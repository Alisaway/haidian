...
| agent.4 AI public space, intelligent-native formats and pilgrimage landmarks | Key areas, blue-green space, component library | Three nodes/landmarks, slow-traffic blue-green public space, reversible and accessible components | key-areas, land-use-structure; `landmark_count`, `public_space_ratio` |
| agent.5 Centennial Jing-Zhang, Zhongguancun and AI-new-culture narrative | Blue-green space, culture, signage and communication | Three-layer narrative, multilingual signage, public supervision and international communication | site-overview, mobility-bluegreen; sources and provisional flags |
...
- **In-belt (taskbook's own structure) — "Three Areas & Two Wings"**: three areas — Zhongzhiyuan AI Autonomous Innovation Accelerator approx. 192 ha, Beijing AI Origin Community approx. 104 ha, Dazhongsi AI Industry Cluster approx. 72 ha; two wings — the Zhongguancun Technology-Service Wing and the Xiaoyuehe Scenario-Empowerment Wing; all official text calibers. This node system is its in-belt coordination counterpart; the key-area detailed design and the three node locations all use "Three Areas & Two Wings" as the in-belt backbone. This node system neither replaces nor redraws "Three Areas & Two Wings". "Three Areas & Two Wings" is the taskbook's official text caliber, and this package does not modify its scope or naming.
...
> **Evidence anchor**: [source:DATA-SRC-AGENT-TASKBOOK] (Three Areas & Two Wings and positioning text), [source:DATA-SRC-PROVISIONAL-BOUNDARIES] ("Five-Region Coordination Network" is this document's own concept name, not an official term).
...
The three node sites are conceptual locations to be relocated after planning, data, heritage and competent-authority assessment and professional review; the green-belt routes and slow-traffic links between nodes form the complete indicator itinerary, expressed in the booklet and the geometry layer. Node mapping to the taskbook's “three areas and two wings” (conceptual): the Indicator Hub takes up the “caliber and service convergence” function of the Zhongguancun Technology-Service Wing (linking the data-governance direction of Future Science City); Monitoring Posts are placed in segments along the green belt (real-time scenario-test coordination with the Zhongzhiyuan Accelerator and the Xiaoyuehe Scenario-Empowerment Wing); the Disclosure Pavilion sits near neighborhoods and rail stations (living-service and public-participation coordination with the Beijing AI Origin Community and the Dazhongsi Cluster). Out-of-belt coordination runs through the “Five-Region Coordination Network” (Beiwai Community, Future Science City, Huairou Science City, the Economic-Technological Development Area and the Jing-Jin-Ji region) for caliber alignment; both loops are dynamically verified through annual recomputation and liaison.
...
The developer-community alliance hosts the Open Day and technical workshops, and open data interfaces are filed and managed, forming an "scenario opening — developer participation — indicator feedback — outcome conversion" mechanism. Developer access follows the Open Indicator Points System · JZ (proposal submission, model submission, defect fixing, event organization all count toward points); points are the basis for prioritized use of pilot nodes and for the annual public acknowledgment. Attraction-conversion follows the "outcome conversion clause" of the Joint Indicator Governance · JZ Compact, defining benefit sharing, attribution and ownership. All access is conditioned on the three-sentence AI governance rule: anonymized aggregation only, human review of key decisions, no excessive monitoring.
...
These cards are the single human-readable source corresponding to `metrics.json.annual_programs`. KPIs are conceptual gates to be confirmed, not current observations. No program starts before resources, privacy, accessibility and venue permissions are verified.
...
- Spatial nodes: Indicator Hub, Disclosure Pavilion and open-data display point (venue `to_verify`).
...
- Preconditions: confirmed dictionary, anonymized aggregates, accessible signage, venue/fire/event permissions (`to_verify`).
- Bilingual touchpoints: Chinese/English caliber cards, captions/handbook and static web page; privacy/accessibility: no identifiable collection, booking alternative and accessible route (details to verify).
- KPI: on-time caliber-card release, public-question closure, accessible-touchpoint coverage (thresholds `to_verify`); gate: human review and resources complete; cancel/exit: cancel public display if any precondition is unconfirmed, archive and return to preparation.
...
- Spatial nodes: Disclosure Pavilion, community deliberation space or online alternative (location `to_verify`).
...
- Bilingual touchpoints: bilingual summary, Q&A record and version number; privacy/accessibility: neighborhood-level aggregates only, anonymous questions, text/voice/caption alternatives (to verify).
- KPI: traceable progress records, timely replies, bilingual-summary release (thresholds `to_verify`); gate: recomputation and human review complete; cancel/exit: defer if data or review is missing and publish no unverified conclusion.

**AP-03 Monitoring Post Experience Week**

- Frequency: one week per year (date `to_verify`); audience: students, volunteers, residents, developers and operators.
- Spatial nodes: Monitoring Post, study terrace and slow-traffic observation points (locations `to_verify`).
...
- Bilingual touchpoints: bilingual task cards, station notes and interpretation; privacy/accessibility: no individual identification or track retention, non-device observation task and assistance option (to verify).
- KPI: booking completion, equipment-fault closure, accessible participation feedback (thresholds `to_verify`); gate: safety and human staffing confirmed; cancel/exit: stop and retrieve equipment if safety, weather or accessibility fails.
...
- Spatial nodes: Indicator Hub, online meeting and bilingual display point at the Pavilion (venue `to_verify`).
...
- Bilingual touchpoints: bilingual method note, case comparison, captions and downloadable version; privacy/accessibility: public or licensed materials only, anonymized aggregate demos, captions/transcript (license details `to_verify`).
- KPI: bilingual-equivalence review, public-response rate, source-ledger completeness (thresholds `to_verify`); gate: rights, facts and human content review pass; cancel/exit: remove material/cancel session and state the reason if permission or facts cannot be verified.
...
> **Evidence anchor**: [source:DATA-SRC-AGENT-TASKBOOK] (phasing and operation items), [data:geometry/key_areas.geojson] (pilot three nodes), [metric:annual_program_count], [metric:phase_count].
...
### Three-sentence AI governance rule (consistent throughout the document)

Anonymized aggregation only — no individual-identification tracking, no always-on recognition devices, no individual behavior profiles retained; key decisions human-reviewed — alert publication, disclosure conclusions and handling recommendations are all human-reviewed; no excessive monitoring — no individual-identification tracking, no individual behavior profiles retained, no large-scale individual-identification monitoring.
...
This package has **not** completed the official trademark search. The original names and marks — "指标智环", "METRIC·JZ", the node names "指标枢 / 监测驿 / 公示亭", and the named concepts "Joint Indicator Governance · JZ Compact" ("指标共治·JZ 公约"), "Open Indicator Points System · JZ" ("指标开放积分制·JZ") and the related visual system — are treated as internal working codenames until the search is completed; they are not used for external registration, formal publication or commercial use, and they do not constitute trademark licensing, exclusive-rights claims or confirmation of protected marks. Any resemblance to existing marks is coincidental; prior-rights search and clearance must precede formal implementation. The logo direction is a concept graphic (ring + three nodes + central axis); no finished logo is delivered in this package and the present review does not evaluate the quality of a finished logo; the formal logo enters design and publication only after passing prior-rights review, and is registered independently under the call rules. Brand recognition and the "METRIC·JZ" core mechanism are built up incrementally through the annual recomputation and the Open Day; no communication or commercialization metrics are presupposed.
...
| Images and charts | site-overview, key-areas, ecosystem-atlas, mobility-bluegreen, metrics-evidence, land-use-structure, logo-metric-jz | Self-produced in this package (AI-assisted generation + matplotlib rendering) | Self-produced; community-display license for this package | Display and review citation in this call-for-entries only; not commercial-grade material |
...
| Brand and visual | "指标智环", "METRIC·JZ" and the named concepts above, plus the visual system | Original concept of this package | Internal working codename | Not used for external registration until prior-rights search completes |
...
> **Evidence anchor**: [source:DATA-SRC-OFFICIAL-ANNOUNCEMENT] (basis for ownership and compliance wording), [source:DATA-SRC-GENERATIVE-AI-INTERIM-MEASURES] (AI-generated content governance).
