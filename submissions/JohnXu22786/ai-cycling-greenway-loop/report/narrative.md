...
本方案「京张AI骑行绿道环(RIDE·JZ LOOP)」以百年京张铁路走廊为骨架,提出一条贯通三区、缝合两翼的环线慢行纽带(骑行+步行)原创概念。方案的机器可读证据链由以下部分构成并相互交叉校验:
...
- **metrics.json**:site_area_sqm、green_ratio、public_space_ratio 三项 formal 核心指标自本包几何在 EPSG:4548 投影下复算,标注 confidence 与复算触发条件;依赖未公开官方控制条件的指标保持 unknown;
